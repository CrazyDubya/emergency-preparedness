...
class EmergencyDrillSimulator:
    ...
    def create_flood_drill(self) -> int:
        """Create a flood drill scenario"""
        objectives = ["Monitor flood warnings", "Prepare home", "Evacuate if necessary"]
        decision_points = [{"phase": "warning", "time_limit": 60, "prompt": "Flood warning issued. What do you do?", "options": {"A": {"action": "Prepare to evacuate", "score": 10, "risk": 0}, "B": {"action": "Ignore it", "score": 0, "risk": 10}}}]
        success_criteria = {"excellent": {"min_score": 8, "max_time": 5, "objectives": 3}, "good": {"min_score": 5, "max_time": 8, "objectives": 2}}
        family_roles = ["Lead", "Supply Manager"]
        return self.save_scenario("Flood Drill", "flood", "beginner", 10, objectives, decision_points, success_criteria, family_roles)

    def create_power_outage_drill(self) -> int:
        """Create a power outage drill scenario"""
        objectives = ["Check for outage cause", "Use backup lighting", "Conserve food"]
        decision_points = [{"phase": "detection", "time_limit": 30, "prompt": "Power is out. What is your first action?", "options": {"A": {"action": "Find flashlight", "score": 10, "risk": 0}, "B": {"action": "Light candles", "score": 5, "risk": 5}}}]
        success_criteria = {"excellent": {"min_score": 8, "max_time": 5, "objectives": 3}, "good": {"min_score": 5, "max_time": 8, "objectives": 2}}
        family_roles = ["Lead", "Safety Officer"]
        return self.save_scenario("Power Outage Drill", "power_outage", "beginner", 10, objectives, decision_points, success_criteria, family_roles)

    def create_wildfire_drill(self) -> int:
        """Create a wildfire drill scenario"""
        objectives = ["Monitor fire proximity", "Prepare to evacuate", "Clear defensible space"]
        decision_points = [{"phase": "evacuation_warning", "time_limit": 60, "prompt": "Evacuation warning issued. What do you do?", "options": {"A": {"action": "Pack go-bags", "score": 10, "risk": 0}, "B": {"action": "Wait and see", "score": 0, "risk": 10}}}]
        success_criteria = {"excellent": {"min_score": 8, "max_time": 5, "objectives": 3}, "good": {"min_score": 5, "max_time": 8, "objectives": 2}}
        family_roles = ["Lead", "Evacuation Coordinator"]
        return self.save_scenario("Wildfire Drill", "wildfire", "intermediate", 15, objectives, decision_points, success_criteria, family_roles)

    def create_hurricane_drill(self) -> int:
        """Create a hurricane drill scenario"""
        objectives = ["Board up windows", "Stockpile water and food", "Evacuate if in mandatory zone"]
        decision_points = [{"phase": "hurricane_watch", "time_limit": 120, "prompt": "Hurricane watch issued. What is your priority?", "options": {"A": {"action": "Secure property", "score": 10, "risk": 0}, "B": {"action": "Ignore it", "score": 0, "risk": 10}}}]
        success_criteria = {"excellent": {"min_score": 8, "max_time": 10, "objectives": 3}, "good": {"min_score": 5, "max_time": 15, "objectives": 2}}
        family_roles = ["Lead", "Supply Manager", "Safety Officer"]
        return self.save_scenario("Hurricane Drill", "hurricane", "advanced", 20, objectives, decision_points, success_criteria, family_roles)

    def create_tsunami_drill(self) -> int:
        """Create a tsunami drill scenario"""
        objectives = ["Recognize tsunami signs", "Evacuate to high ground", "Stay informed"]
        decision_points = [{"phase": "earthquake_felt", "time_limit": 30, "prompt": "You are at the beach and feel a strong earthquake. What do you do?", "options": {"A": {"action": "Move to high ground immediately", "score": 10, "risk": 0}, "B": {"action": "Wait for official warning", "score": 2, "risk": 8}}}]
        success_criteria = {"excellent": {"min_score": 8, "max_time": 5, "objectives": 3}, "good": {"min_score": 5, "max_time": 8, "objectives": 2}}
        family_roles = ["Lead", "Evacuation Coordinator"]
        return self.save_scenario("Tsunami Drill", "tsunami", "advanced", 10, objectives, decision_points, success_criteria, family_roles)

    def create_volcanic_eruption_drill(self) -> int:
        """Create a volcanic eruption drill scenario"""
        objectives = ["Monitor eruption status", "Protect from ashfall", "Evacuate if ordered"]
        decision_points = [{"phase": "ashfall_warning", "time_limit": 60, "prompt": "Ashfall warning issued. What is your first action?", "options": {"A": {"action": "Seal windows and doors", "score": 10, "risk": 0}, "B": {"action": "Go outside to watch", "score": 0, "risk": 10}}}]
        success_criteria = {"excellent": {"min_score": 8, "max_time": 5, "objectives": 3}, "good": {"min_score": 5, "max_time": 8, "objectives": 2}}
        family_roles = ["Lead", "Safety Officer"]
        return self.save_scenario("Volcanic Eruption Drill", "volcanic_eruption", "advanced", 15, objectives, decision_points, success_criteria, family_roles)

    def create_pandemic_drill(self) -> int:
        """Create a pandemic drill scenario"""
        objectives = ["Practice hygiene", "Maintain social distance", "Manage supplies for lockdown"]
        decision_points = [{"phase": "lockdown_announced", "time_limit": 120, "prompt": "A city-wide lockdown is announced. What is your priority?", "options": {"A": {"action": "Inventory supplies", "score": 10, "risk": 0}, "B": {"action": "Visit friends", "score": 0, "risk": 10}}}]
        success_criteria = {"excellent": {"min_score": 8, "max_time": 10, "objectives": 3}, "good": {"min_score": 5, "max_time": 15, "objectives": 2}}
        family_roles = ["Lead", "Supply Manager", "Health Officer"]
        return self.save_scenario("Pandemic Drill", "pandemic", "intermediate", 20, objectives, decision_points, success_criteria, family_roles)
...
if __name__ == "__main__":
    ...
    flood_id = simulator.create_flood_drill()
    power_outage_id = simulator.create_power_outage_drill()
    wildfire_id = simulator.create_wildfire_drill()
    hurricane_id = simulator.create_hurricane_drill()
    tsunami_id = simulator.create_tsunami_drill()
    volcanic_eruption_id = simulator.create_volcanic_eruption_drill()
    pandemic_id = simulator.create_pandemic_drill()
    ...
    print(f"  4. Flood Drill (ID: {flood_id})")
    print(f"  5. Power Outage Drill (ID: {power_outage_id})")
    print(f"  6. Wildfire Drill (ID: {wildfire_id})")
    print(f"  7. Hurricane Drill (ID: {hurricane_id})")
    print(f"  8. Tsunami Drill (ID: {tsunami_id})")
    print(f"  9. Volcanic Eruption Drill (ID: {volcanic_eruption_id})")
    print(f"  10. Pandemic Drill (ID: {pandemic_id})")
    
    # Run a drill
    choice = input("\nSelect drill to run (1-10): ")
    name = input("Your name: ")
    family = int(input("Family size (1-5): ") or 4)
    
    drill_map = {
        '1': earthquake_id, '2': fire_id, '3': tornado_id, '4': flood_id,
        '5': power_outage_id, '6': wildfire_id, '7': hurricane_id,
        '8': tsunami_id, '9': volcanic_eruption_id, '10': pandemic_id
    }
