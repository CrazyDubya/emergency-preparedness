...
class KnowledgeBaseSearch:
    def __init__(self, knowledge_dir: str = "disaster_knowledge_base", db_path: str = "knowledge_base.db"):
        ...
        print(f"DEBUG: KnowledgeBaseSearch using DB path: {self.db_path}")
        self.init_database()
        self.index_built = True # Assume index is built by build_knowledge_base.py

        # Check if the knowledge base is populated, if not, build it
        if self._get_document_count() == 0:
            print("Knowledge base is empty. Building vector store...")
            build_vector_store(self.knowledge_dir, self.db_path)
    ...
    # The index_knowledge_base method is now handled by build_knowledge_base.py
    # This class assumes the database is already populated.
    ...
    def search(self, query: str, category: Optional[str] = None, limit: int = 10) -> List[Dict]:
        ...
        print(f"DEBUG: Search query tokenized to: {terms}") # DEBUG
        ...
        for term in terms:
            ...
            print(f"DEBUG: Executing SQL: {sql} with params: {params}") # DEBUG
        ...
        conn.close()
        print(f"DEBUG: Raw search results: {results}") # DEBUG
        # Sort by relevance and return top results
        sorted_results = sorted(results.values(), key=lambda x: x['relevance_score'], reverse=True)
        print(f"DEBUG: Sorted search results: {sorted_results}") # DEBUG
