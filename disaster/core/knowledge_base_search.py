...
class KnowledgeBaseSearch:
    def __init__(self, knowledge_dir: str = "knowledge_base", db_path: str = "knowledge_base.db"):
    ...
    def get_documents_by_category(self, category: str, limit: int = 20) -> List[Dict]:
        """Get documents in a category (for browse). Uses folder path matching."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('''
            SELECT id, title, file_path, category FROM knowledge_documents
            WHERE file_path LIKE ? OR category = ?
            ORDER BY title LIMIT ?
        ''', (f"%{category}%", category, limit))
        rows = cursor.fetchall()
        conn.close()
        return [{"id": r[0], "title": r[1], "file_path": r[2], "category": r[3]} for r in rows]
    ...
    # Scenario name -> filename mapping for quick-help
    SCENARIO_FILES = {
        "earthquake": "earthquake_plan.md",
        "flood": "flood_safety.md",
        "hurricane": "hurricane_preparedness.md",
        "tornado": "tornado_preparedness.md",
        "winter_storm": "winter_storm_preparedness.md",
        "power_failure": "power_failure_plan.md",
        "power": "power_failure_plan.md",
        "extreme_heat": "extreme_heat_preparedness.md",
        "heat": "extreme_heat_preparedness.md",
        "tsunami": "tsunami_preparedness.md",
        "landslide": "landslide_preparedness.md",
        "pandemic": "pandemic_preparedness.md",
        "active_shooter": "active_shooter_response.md",
        "commuter": "commuter_emergency_plan.md",
    }
    
    def get_scenario_guide(self, scenario_name: str) -> Optional[Dict]:
        """
        Get scenario guide content by name (e.g. flood, earthquake, hurricane).
        Returns dict with title, content, file_path or None if not found.
        """
        name = scenario_name.lower().strip().replace(" ", "_")
        filename = self.SCENARIO_FILES.get(name)
        if not filename:
            # Try direct filename match
            if name.endswith(".md"):
                filename = name
            else:
                filename = f"{name}_preparedness.md"
        base = Path(self.knowledge_dir) / "scenario_specific_plans"
        candidates = [
            base / filename,
            base / f"{name}.md",
            base / f"{name}_plan.md",
            base / f"{name}_safety.md",
        ]
        for path in candidates:
            if path.exists():
                try:
                    content = path.read_text(encoding="utf-8")
                    title = self._extract_title(content) or path.stem.replace("_", " ").title()
                    return {
                        "title": title,
                        "content": content,
                        "file_path": str(path),
    ...
    def list_scenario_guides(self) -> List[str]:
        """List available scenario guide names (from files on disk)"""
        base = Path(self.knowledge_dir) / "scenario_specific_plans"
        names = set()
        if base.exists():
            for p in base.glob("*.md"):
                stem = p.stem.replace("_preparedness", "").replace("_plan", "").replace("_safety", "")
                names.add(stem.replace("_", " "))
        return sorted(names)
    
    def list_categories(self) -> List[Dict]:
        """List knowledge base categories (from folder structure) with display names"""
        kb_path = Path(self.knowledge_dir)
        if not kb_path.exists():
            return []
        category_map = {
            "scenario_specific_plans": "Scenario Plans",
            ...
            "security": "Security",
            "sanitation_and_hygiene": "Sanitation & Hygiene",
            "psychology_and_community": "Psychology & Community",
            "long_term_sustainability": "Long-Term Sustainability",
            "modern_threats": "Modern Threats",
            "barter_and_trade": "Barter & Trade",
        ...
        for d in sorted(kb_path.iterdir()):
            if d.is_dir() and not d.name.startswith("."):
                display = category_map.get(d.name, d.name.replace("_", " ").title())
                count = len(list(d.rglob("*.md"))) + len(list(d.rglob("*.pdf")))
                result.append({"id": d.name, "name": display, "doc_count": count})
        return result
