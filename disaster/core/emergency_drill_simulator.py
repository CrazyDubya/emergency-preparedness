...
class EmergencyDrillSimulator:
    ...
    def run_drill_quiz(
        self, scenario_id: int, participant_name: str, family_size: int,
        choices: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        Run drill without user input (for web/API). Uses choices if provided,
        otherwise selects best option (A) for each decision.
        """
        if choices is None:
            choices = []
        conn = sqlite3.connect(self.db_path)
        ...
        if not row:
            raise DrillScenarioNotFoundError(scenario_id)
        scenario = {
            "id": row[0], "name": row[1], "disaster_type": row[2],
            "difficulty": row[3], "duration_minutes": row[4],
            "objectives": json.loads(row[5]), "decision_points": json.loads(row[6]),
            "success_criteria": json.loads(row[7]), "family_roles": json.loads(row[8])
        }
        start_time = datetime.now()
        total_score = 0
        decisions_made = []
        ...
        for i, decision in enumerate(scenario["decision_points"]):
            choice = choices[i] if i < len(choices) and choices[i] in decision["options"] else "A"
            option_chosen = decision["options"][choice]
            total_score += option_chosen["score"]
            total_risk += option_chosen["risk"]
            decisions_made.append({
                "phase": decision["phase"], "choice": choice,
                "action": option_chosen["action"], "score": option_chosen["score"],
                "risk": option_chosen["risk"], "time": 0.5
            })
        end_time = datetime.now()
        total_time = (end_time - start_time).total_seconds() / 60.0
        obj_count = min(7, max(1, total_score // 5))
        objectives_completed = scenario["objectives"][:obj_count]
        performance_grade = "needs_improvement"
        for grade, criteria in scenario["success_criteria"].items():
            if total_score >= criteria["min_score"] and len(objectives_completed) >= criteria["objectives"]:
                performance_grade = grade
                break
        lessons = [f"Good: {d['action']}" for d in decisions_made if d["score"] >= 9]
        if total_risk > 20:
            lessons.append("Reduce risk-taking - safety first!")
        drill_result_id = self.save_drill_result(
            scenario_id, participant_name, family_size,
            start_time.isoformat(), end_time.isoformat(),
            decisions_made, objectives_completed,
        ...
        return {
            "drill_result_id": drill_result_id, "score": total_score,
            "grade": performance_grade, "time": total_time,
            "decisions": decisions_made, "lessons": lessons, "success": True
        }
    ...
    def list_scenarios(self) -> List[Dict]:
        """List all available drill scenarios for API/UI"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('''
            SELECT id, name, disaster_type, difficulty, duration_minutes
            FROM drill_scenarios
            ORDER BY disaster_type, name
        ''')
