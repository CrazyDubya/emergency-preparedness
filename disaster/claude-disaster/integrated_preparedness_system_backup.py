    ...
    def run_advanced_risk_engine(self):
        ...
        while True:
            ...
            if choice == "0":
                ...
                print(f"Probability: {risk_result['probability_score']:.1f}%")
                print(f"Impact Severity: {risk_result['impact_score']:.1f}/10")
                print(f"Vulnerability: {risk_result['vulnerability_score']:.1f}/10")
                print(f"Preparedness Level: {risk_result['preparedness_score']:.1f}/10")
                
                if risk_result['risk_factors']:
                    print(f"\n🎯 Key Risk Factors:")
                    for factor in risk_result['risk_factors'][:5]:
                        print(f"  • {factor['factor']}: {factor['weight']:.1f} (Impact: {factor['contribution']:.1f})")
                        
                if risk_result['mitigation_recommendations']:
                    print(f"\n💡 Mitigation Recommendations:")
                    for rec in risk_result['mitigation_recommendations'][:3]:
                        print(f"  • {rec}")
            elif choice == "7":
